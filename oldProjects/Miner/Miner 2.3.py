...
print("RedStar Corporation | discord - green_tea_bag | version: 2.3 | 334 строки кода")
...
def Mine(Counts):
    ...
    for i in range(Counts):
        ...
        if FireMiner == True:
            FireMoney += 0.5
            print("system> Добавлено 0.5 FireMoney на счёт")
...
while True:
    ...
    # load
        ...
        if (FireMoney != 0) or (FireMiner == True):
            print("system> FireMoney будет стёрт.")
        UserAnswer = input("\nuser> ")
        if (UserAnswer == "Add+100PMC_Now") and (cheats):
            ...
            if UserAnswer == "U015XzgtUE1DLVNNZF8xMl9TUy0xLTE=":
                print("\nsystem> Предположитеьно что-то идёт не так.")
    ...
    # Бессмысленно занимает много места, но и в одну строку выглядело бы глупо, а отдельный файл - не вариант
        ...
        print("\nsystem> Вот весь список обновлений:") # Не уносить
        print("\n-----= 2.3 =-----\n")
